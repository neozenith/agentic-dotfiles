...
| **DT-PROV-1** | Each IR value keeps what curation derived; a value that differs is a stated hand edit, kept and fed forward; untouched values follow the seed |
| **DT-WALK-2** | Walk lightness is solved per slot and mode; chroma defaults uniform, with `walk.chromaCeiling: "seed"` for a truer-to-brand walk |
