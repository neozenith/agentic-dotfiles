...
### Using with render_mermaid.sh

When rendering markdown files that contain `architecture-beta` diagrams with Iconify
icons, pass `--iconPacks` directly to `mmdc`. The bundled `render_mermaid.sh` script
does not currently support icon packs — use `mmdc` directly for these diagrams:

```bash
npx -p @mermaid-js/mermaid-cli mmdc \
  -i document.md \
  -a .mmdc_cache/dark_transparent_png/ \
  -o .mmdc_cache/dark_transparent_png/document.md \
