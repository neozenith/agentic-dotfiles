...
# Pyodide (CPython in WebAssembly) runs a theme's own generator for live seed tuning
# (ADR-022). Showcase-only, so it is pinned here rather than in the viewer's CDN table;
# its runtime files resolve beside pyodide.js, and it loads only when a control changes.
PYODIDE_CDN = "https://cdn.jsdelivr.net/npm/pyodide@314.0.7/pyodide.js"
...
def _load_lineage(theme: Theme) -> dict[str, Any] | None:
    """A theme generated by a token pipeline may ship `lineage.json` beside its brandpack:
    Sankey nodes and links tracing where each value came from. Hand-authored themes have
    none, and the showcase says so rather than inventing one (ADR-021)."""
    path = theme.tokens_path.parent / "lineage.json"
    if not path.is_file():
        return None
    data: dict[str, Any] = json.loads(path.read_text(encoding="utf-8"))
    for s in data.get("sankeys", []):
        ids = {n["id"] for n in s["nodes"]}
        for link in s["links"]:
            if link["source"] not in ids or link["target"] not in ids:
                raise SystemExit(f"error: {path}: link {link} names an unknown node")
    return data


def _load_generator(theme: Theme) -> dict[str, Any] | None:
    """A generated theme may also ship its producer: `generator.json` (the seed, and a range per
    parameter) plus `generator.py`, a stdlib-only module whose `generate(seed)` returns
    `{"tokens": <brandpack>, "lineage": <optional>}`. The showcase runs it in the browser under
    Pyodide so the seed can be tuned live (ADR-022). richdocs never runs it itself."""
    folder = theme.tokens_path.parent
    manifest_path, source_path = folder / "generator.json", folder / "generator.py"
    if not manifest_path.is_file():
        return None
    if not source_path.is_file():
        raise SystemExit(f"error: {manifest_path} has no generator.py beside it")
    manifest: dict[str, Any] = json.loads(manifest_path.read_text(encoding="utf-8"))
    for key in ("entry", "seed", "params"):
        if key not in manifest:
            raise SystemExit(f"error: {manifest_path}: missing {key!r}")
        ...
        "brands": [
            {
                ...
                "lineage": _load_lineage(t),
                "generator": _load_generator(t),
            }
            for t in themes
        ],
        "architectures": [
