...
- **Claim (skill's first fix):** the mermaid parse/contrast gate should be
  satisfied by pointing SKILL.md at the sibling `mermaidjs-diagrams` skill's
...
- **Ruling (user):** rejected, emphatically. richdocs must maintain a
  **wholesale vendored copy** of the mermaid toolchain
  (`vendor/mermaidjs-diagrams/`) so it stands on its own. richdocs must NOT
...
- **How applied:** vendor copy at `vendor/mermaidjs-diagrams/`; all sibling
