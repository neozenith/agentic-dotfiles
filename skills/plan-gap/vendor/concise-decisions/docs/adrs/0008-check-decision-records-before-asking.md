...
| Field | Value |
|---|---|
| **Status** | Accepted · 2026-08-23 · backend 3 superseded by [ADR-0020](0020-the-adr-log-is-the-only-knowledge-store.md) |
| **Provenance** | user instruction, 2026-08-23 — self-answer from existing decision records |
...
- Records are a **role** with four backends in order: the ADR surface and CLAUDE.md lenses; the plan's decisions and
  ...
  *(Backend 3 superseded by [ADR-0020](0020-the-adr-log-is-the-only-knowledge-store.md): the skill keeps no
  adjudications of its own, so it reads as the user's prior feedback this session and persisted memory. The rule that
  the search runs before ranking is untouched.)*
