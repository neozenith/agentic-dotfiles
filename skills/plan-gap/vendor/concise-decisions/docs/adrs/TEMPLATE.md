...
- **Accepted ADRs are immutable in substance.** Reformatting to a new template shape is not a change of mind; a change
  of mind is a new ADR that supersedes the old one, with links both ways. Supersession may be **partial**: a later ADR
  can retire one clause and leave the rest standing. Record that in the earlier ADR's Status row, its `Relates to`
  row, and a parenthetical marker on the affected bullet. Never by rewriting the clause.
- **The ADR is the whole record.** Write the user's own words into `Problem` here; there is no companion ledger, log,
  or learning file to hold the evidence, and adding one fragments the store (ADR-0020).
