...
re-deriving the trade-off. The list below is all nineteen; open a file only when you need its argument.
...
All nineteen are **Accepted**; none has been superseded. Dates and provenance live in each file's metadata table.
...
| 0012 | `SKILL.md` steps 4, 4.5, 6 · § Never · `harnesses/*.md` |
...
| 0018 | `SKILL.md` § Resources · § Decision records row 3 |
...
3. If it supersedes an earlier ADR, set that one's Status to `Superseded by ADR-NNNN` and link both ways.
4. Promotions from [`adjudications.md`](../../resources/learned/adjudications.md) cite the `ADJ-NN` id in
   `Provenance` and leave the adjudication in place as evidence.
