...
| `resources/learned/adjudications.md` | `ADJ-NN` evidence trail of user rulings; every entry cites its ADR | **yes**, first question |
...
| `docs/skill-loading-map.md` | Debug aid: what loads, when, on which condition | no |
...
Accepted ADRs are immutable in substance — supersede with a new file and a cross-link, never an edit. Reformatting to
a new template shape is not a change of mind.

Current lenses, one line each: two surfaces (0001) · no cheap form (0002) · TBD is a route family (0003) ·
shapes/harnesses are lazy files (0004) · permutations, never multi-select (0005) · text feed is first-class (0006) ·
the five checks are the rubric (0007) · search records before asking (0008) · a TBD route leaves content, not a
pointer (0009) · headings index the hierarchy (0010) · a lens has a `Given` and an `Unless` (0011) · one decision per
turn (0012) · free text is the contract (0013) · previews are complete outcomes on real data (0014) · the briefing is
scanned, not read (0015) · the cascade carries the lens (0016) · the reply grammar is two prose lines (0017) · runtime
authority is `SKILL.md` + `resources/` (0018) · the ranking is shown (0019).
...
- [ ] User override during a run → `resources/learned/adjudications.md` in the same turn, with the next `ADJ-NN` id
      and the user's own words.
- [ ] Promotion → `docs/adrs/NNNN-*.md` from [TEMPLATE.md](docs/adrs/TEMPLATE.md), a row in **both** index tables, the
      `ADJ-NN` in `Provenance`, **and the runtime surfaces named in `Enforced in` changed in the same commit.** An ADR
      whose surfaces were not updated is the drift the log exists to prevent.
