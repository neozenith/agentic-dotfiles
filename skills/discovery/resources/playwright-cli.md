...
Reference for using `playwright-cli` to verify external URLs cited in discovery
...
>
> After installing, re-run `/discovery <path>` and ask to retry link verification.

Do not block on user input. Detect once at the start of the link-verification step,
inform the user, and proceed with the best available tier for the entire verification
batch.
